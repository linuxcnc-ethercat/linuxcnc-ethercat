...
/// @file
/// @brief Driver for RTelligent EC* EtherCAT stepper drives
...
// Constants for modparams.
#define MODPARAM_CHANNEL_MASK 0xf
...
// XXXX: remove `rtec` and replace it with your device name,
...
static lcec_typelist_t types[] = {
    {"ECR60x2x", LCEC_RTELLIGENT_VID, 0x0a880005, 0, NULL, lcec_rtec_init, NULL, F_AXES(2) | F_PDOINCREMENT(0x10) | F_NOEXTRAS},
    {"ECT60x2x", LCEC_RTELLIGENT_VID, 0x0a880006, 0, NULL, lcec_rtec_init, NULL, F_AXES(2) | F_PDOINCREMENT(0x10) | F_NOEXTRAS},
    {NULL},
};
ADD_TYPES_WITH_CIA402_MODPARAMS(types, modparams_perchannel, modparams_base, overrides)

// Lookup tables for modParams
...
static int handle_modparams(lcec_slave_t *slave, lcec_class_cia402_options_t *options) {
  ...
  // Input and output polarity are implemented as a bitmask.  We need
  // to support changing a single bit via modParam, so we want to read
  // in the current values so setting 1 bit doesn't reset all of the
  // others.
  ...
  for (p = slave->modparams; p != NULL && p->id >= 0; p++) {
    // CiA 402 devices with multiple channels let you pass some
    // modparams for individual channels.  Under the hood, this is
    // done by creating multiple modParam entries with slightly
    // different names ("ch1Foo", "ch2Foo", etc) and incrementing the
    // modParam ID by one for each.  This uses MODPARAM_CHANNEL_MASK
    // to break the per-channel part and the channel-independent parts
    // of the modParam ID apart so we don't need multiple cases for
    // each.
    int channel = p->id & MODPARAM_CHANNEL_MASK;
    int id = p->id & ~MODPARAM_CHANNEL_MASK;
    int base = 0x2000 + 0x800 * channel;

    // RT EC* devices only come in 1 and 2 channel forms.
