...
// DC synchronisation: Master-to-Reference-clock (M2R) mode.
// LinuxCNC servo thread syncs to EtherCAT reference clock via RTAPI PLL.
...
#define DC_SETTLE_TIME       1.5
#define DC_DAMPING           0.707
#define DC_INTEGRATOR_MAX    1000.0
#define DC_CORRECTION_MAX_NS 5000.0
...
static void pre_send(struct lcec_master *master) {
  ...
  // capture current time for next cycle prediction
